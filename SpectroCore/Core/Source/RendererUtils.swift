...
enum GLRendererUtils {
    ...
    static func loadShaderProgram(_ name: String, bundle: Bundle) -> GLuint {
        
        let vertexShader = compiledShader(type: GLenum(GL_VERTEX_SHADER), sourceName: name, bundle: bundle)
        let fragmentShader = compiledShader(type: GLenum(GL_FRAGMENT_SHADER), sourceName: name, bundle: bundle)
...
private func compiledShader(type: GLenum, sourceName: String, bundle: Bundle) -> GLuint {
    
    let fileExtension = (type == GL_VERTEX_SHADER) ? "vsh" : "fsh"
    let url = bundle.url(forResource: sourceName, withExtension: fileExtension)!
